...
\begin{enumerate}
  \item Simplify: $x^5 \cdot x^7$
  \vspace{\stretch{1}}
  \item Simplify: $\dfrac{a^{12}}{a^3}$
  \vspace{\stretch{1}}
  \item Simplify: $(y^4)^3$
  \vspace{\stretch{1}}
  \item Rewrite with positive exponents only: $b^{-6}$\\
  (Hint: Use a fraction)
  \vspace{\stretch{1}}
    \clearpage
  \item Evaluate without a calculator: $32^{1/5}$
  \vspace{\stretch{1}}
  \item Simplify completely: $\left( \dfrac{3x^2}{y^3} \right)^2$
  \vspace{\stretch{1}}
\end{enumerate}
\section*{6. Exit Tickets}
An exponential function is of the form $f(x) = ab^x$. Given $f(0) = 25$ and $f(1) = 20$, find $f(x)$.
  \vspace{\stretch{1}}
